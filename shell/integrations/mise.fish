...
function mise
  ...
  if contains -- --help $argv
    command /run/current-system/sw/bin/mise "$command" $argv
    return $status
  end
  ...
  case deactivate shell sh
    ...
    if contains -- -h $argv
      ...
      source (command /run/current-system/sw/bin/mise "$command" $argv |psub)
    end
  case '*'
    command /run/current-system/sw/bin/mise "$command" $argv
  end
...
function __mise_env_eval --on-event fish_prompt --description 'Update mise environment when changing directories';
    /run/current-system/sw/bin/mise hook-env -s fish | source;
...
function fish_command_not_found
    if string match -qrv -- '^(?:mise$|mise-)' $argv[1] &&
        /run/current-system/sw/bin/mise hook-not-found -s fish -- $argv[1]
        /run/current-system/sw/bin/mise hook-env -s fish | source
