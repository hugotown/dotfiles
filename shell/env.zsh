...
if command -v sops >/dev/null 2>&1 && [ -f "$HOME/.config/nixos/secrets/gemini_api_key.yaml" ]; then
    export GEMINI_API_KEY="$(sops -d "$HOME/.config/nixos/secrets/gemini_api_key.yaml" | yq '.GEMINI_API_KEY' | tr -d '\n')"
    export GOOGLE_GENERATIVE_AI_API_KEY="$GEMINI_API_KEY"
fi
if command -v sops >/dev/null 2>&1 && [ -f "$HOME/.config/nixos/secrets/google_api_key.yaml" ]; then
