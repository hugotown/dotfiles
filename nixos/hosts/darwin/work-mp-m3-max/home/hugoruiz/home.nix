{ config, inputs, pkgs, lib, hostname, ... }:
{
  ...
  home.activation.generateShellIntegrations = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    if [ -x "${pkgs.zoxide}/bin/zoxide" ]; then
      $DRY_RUN_CMD ${pkgs.zoxide}/bin/zoxide init fish > $HOME/.zoxide.fish 2>/dev/null && echo "  ✅ .zoxide.fish"
      $DRY_RUN_CMD ${pkgs.zoxide}/bin/zoxide init nushell > $HOME/.zoxide.nu 2>/dev/null && echo "  ✅ .zoxide.nu"
      $DRY_RUN_CMD ${pkgs.zoxide}/bin/zoxide init zsh > $HOME/.zoxide.zsh 2>/dev/null && echo "  ✅ .zoxide.zsh"
      $DRY_RUN_CMD ${pkgs.zoxide}/bin/zoxide init bash > $HOME/.zoxide.bash 2>/dev/null && echo "  ✅ .zoxide.bash"
    else
    ...
    if [ -x "${pkgs.starship}/bin/starship" ]; then
      ...
      $DRY_RUN_CMD ${pkgs.starship}/bin/starship init nu > $HOME/.starship.nu 2>/dev/null && echo "  ✅ .starship.nu"
      $DRY_RUN_CMD ${pkgs.starship}/bin/starship init zsh > $HOME/.starship.zsh 2>/dev/null && echo "  ✅ .starship.zsh"
      $DRY_RUN_CMD ${pkgs.starship}/bin/starship init bash > $HOME/.starship.bash 2>/dev/null && echo "  ✅ .starship.bash"
    else
      echo "  ⚠️  starship no encontrado"
    fi
...
EOF_NU_CONFIG
    ...
    echo "  ✅ Nushell env.nu → Managed by xdg.configFile (includes secrets)"
...
EOF_ZSH_CONFIG
    ...
    if [ -f "$HOME/.config/nushell/config.nu" ]; then
      ...
      if ! grep -q "source ~/.starship.nu" "$HOME/.config/nushell/config.nu" 2>/dev/null; then
        ...
        echo "  ✅ Nushell: Agregado source ~/.starship.nu"
      fi
