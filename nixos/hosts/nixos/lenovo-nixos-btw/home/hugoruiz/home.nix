...
EOFNUENV
  ...
  # ===== POST-ACTIVATION HOOK: FISH ENV CONFIGURATION =====
  home.activation.configureFishEnv = lib.hm.dag.entryAfter ["linkGeneration" "reloadSystemd"] ''
    echo "🐚 Configurando Fish environment..."

    # Crear directorio de config de fish si no existe
    mkdir -p "$HOME/.config/fish/conf.d"

    # Generar 00-nix-env.fish con PATH y secretos
    $DRY_RUN_CMD cat > $HOME/.config/fish/conf.d/00-nix-env.fish << 'EOFFISH'
...
EOFFISH
  ...
  # ===== POST-ACTIVATION HOOK: ZSH ENV CONFIGURATION =====
  home.activation.configureZshEnv = lib.hm.dag.entryAfter ["linkGeneration" "reloadSystemd"] ''
    echo "🐚 Configurando Zsh environment..."

    # Generar .zshrc.secrets con PATH y secretos
    $DRY_RUN_CMD cat > $HOME/.zshrc.secrets << 'EOFZSH'
# Zsh Environment Configuration
# Auto-generated by home-manager - DO NOT EDIT MANUALLY

# Configure PATH for NixOS
export PATH="$HOME/.cargo/bin:$HOME/.local/bin:/nix/var/nix/profiles/default/bin:$HOME/.nix-profile/bin:/run/current-system/sw/bin:$PATH"
