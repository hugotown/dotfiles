...
- **Global scope**: `/Users/hugoruiz/.config/opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/hugoruiz/.config/opencode/notes/`)
...
2. Glob `/Users/hugoruiz/.config/opencode/notes/*.md` (if directory exists) — global notes
...
Global (/Users/hugoruiz/.config/opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/hugoruiz/.config/opencode/notes/` — works in any directory
