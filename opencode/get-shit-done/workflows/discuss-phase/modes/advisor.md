...
> `/Users/hugoruiz/.config/opencode/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "/Users/hugoruiz/.config/opencode/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @/Users/hugoruiz/.config/opencode/agents/gsd-advisor-researcher.md for your role and instructions.
