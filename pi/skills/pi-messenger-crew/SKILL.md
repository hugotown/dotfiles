...
pi_messenger({
  action: "plan",
  prompt: "Scan the codebase for bugs focusing on error handling",
});

// Re-plan with a steering prompt (wipes existing tasks, preserves progress notes)
pi_messenger({
  action: "plan",
  prompt: "Split the auth module into login and registration",
});
...
pi_messenger({
  ...
  id: "task-1",
  message: "Implemented auth middleware",
});
...
pi_messenger({
  action: "task.revise-tree",
  id: "task-3",
  prompt: "split this into separate API and CLI tasks",
});
```
