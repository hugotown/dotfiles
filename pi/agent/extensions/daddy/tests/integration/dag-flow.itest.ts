...
function project(): RunDeps {
  ...
  fs.mkdirSync(path.join(proj, ".daddy", "workflows"), { recursive: true });
  const { provider, model } = defaultModel();
  fs.writeFileSync(path.join(proj, ".daddy", "workflows", "demo.yaml"),
    "name: demo\ndescription: e2e\nnodes:\n" +
  ...
  return {
    ...
    home: path.join(proj, ".daddy"), bundledDir: path.join(proj, ".daddy"),
