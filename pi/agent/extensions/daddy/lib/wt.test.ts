...
test("wtCreate switches then resolves path", async () => {
  const exec = fakeExec({
    "wt switch --create daddy/x": { code: 0 },
    "wt list --format=json": { stdout: JSON.stringify([{ branch: "daddy/x", path: "/wt/x" }]) },
  });
  expect((await wtCreate(exec, "daddy/x", "/repo")).path).toBe("/wt/x");
});
...
test("wtRemove issues remove with flags", async () => {
  ...
  await wtRemove(exec, "daddy/x", "/repo");
  expect(called).toBe("wt remove daddy/x --yes --force");
});
