...
export async function syncSettings(): Promise<void> {
  ...
  if (opencodeGo.length + opencode.length === 0) {
    log("warn", "both providers returned zero model IDs; skipping write to settings.json");
    return;
  }
  ...
  if (!Array.isArray(settings.enabledModels)) {
    log("warn", "settings.json has no 'enabledModels' array; skipping write");
    return;
  }
  ...
  if (merged.length === previous.length && merged.every((id, i) => id === previous[i])) {
    log("info", "enabledModels list already matches available models; nothing to write");
    return;
  }
  ...
  log("info", `updated 'enabledModels' in settings.json: added ${added.length} model IDs, removed ${removed.length}`);
