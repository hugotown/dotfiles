// Pure 24-bit truecolor painting helpers + width padding. Color values come from
// the resolved ThemeColors (lib/theme.ts), loaded from config.yml. fg/bg reset only
// their own channel, so wrapping a padded line in bg() yields a solid, opaque row.
