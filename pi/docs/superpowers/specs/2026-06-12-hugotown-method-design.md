# hugotown-method — Design Specification
...
| Trigger mechanism | `/hugotown-method flow=<name> <args>` explicit command |
| Workflow storage | Bundled in extension + `.hugotown/workflows/` project override |
...
| Persistence | Hybrid: state files (`.hugotown/runs/`) + session events for TUI |
...
| Commands system | `.hugotown/commands/` project + extension bundled, .md templates |
...
```
START    → wt switch --create hugotown/<workflow>-<short-id>
EXECUTE  → all nodes run in worktree CWD
COMPLETE → prompt user:
           /hugotown-method merge   → wt merge --yes
           /hugotown-method remove  → wt remove --yes --force
           /hugotown-method keep    → leave for manual review
```

Branch naming: `hugotown/<workflow-name>-<6-char-hash>`
...
export default function hugotownMethod(pi: ExtensionAPI): void {
  pi.registerCommand("hugotown-method", { ... });
  pi.registerTool({ name: "hugotown_method", ... });
...
| Command | Action |
|---------|--------|
| `/hugotown-method flow=<name> <args>` | Execute workflow |
| `/hugotown-method list` | List available workflows |
| `/hugotown-method status` | Active/paused runs |
| `/hugotown-method resume <id>` | Resume failed run |
| `/hugotown-method approve [comment]` | Approve pending gate |
| `/hugotown-method reject [reason]` | Reject pending gate |
| `/hugotown-method merge` | Merge current worktree |
| `/hugotown-method remove` | Remove worktree |
| `/hugotown-method keep` | Keep worktree |
| `/hugotown-method validate <name>` | Validate workflow YAML |
...
Flow: emit inline message → pause → wait for `/hugotown-method approve` or `reject` → resume.

## State Persistence

### Run State File (`.hugotown/runs/<id>.json`)
...
```json
{
  ...
  "worktree": { "branch": "hugotown/fix-issue-abc123", "path": "/path" },
  "started_at": "ISO",
  "completed_at": "ISO",
  "nodes": {
    "classify": {
